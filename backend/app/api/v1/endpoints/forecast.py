...
async def create_forecast(
    request: ForecastRequest, current_user=Depends(get_current_user)
):
    ...
    try:
        ...
        last_date = dates[-1]
        future_dates = ForecastService.generate_future_dates(
            last_date, request.forecast_days
        )

        # Format response data
        historical_data = ForecastService.format_historical_data(
            prices, dates, display_count=100
        )
        ...
        return ForecastResponse(
            ...
            model_info={"type": "LSTM", "epochs": train_metrics["epochs"]},
        )
...
async def compare_forecasts(
    ...
    try:
        ...
        transformer_forecaster, transformer_metrics = (
            ForecastService.train_transformer_model(
                prices, request.window_size, request.forecast_days, epochs=50
            )
        )
        ...
        lstm_prediction_data = ForecastService.format_predictions(
            lstm_predictions, future_dates
        )
        transformer_prediction_data = ForecastService.format_predictions(
            transformer_predictions, future_dates
        )
        ...
        print(f"\nError: {str(e)}\n")
        raise HTTPException(
            status_code=500, detail=f"Forecast comparison failed: {str(e)}"
        )
