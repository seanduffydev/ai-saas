...
class AlphaVantageNewsFetcher:
    ...
    def fetch_news(commodity: str, api_key: str, limit: int = 10) -> list[dict]:
        ...
        AlphaVantageNewsFetcher.COMMODITY_KEYWORDS.get(
            commodity.lower(), f"{commodity} price"
        )
        ...
        try:
            response = requests.get(
                AlphaVantageNewsFetcher.BASE_URL, params=params, timeout=10
            )
            ...
            patterns = commodity_patterns.get(
                commodity.lower(), [r"\b" + commodity.lower() + r"\b"]
            )
            ...
            # If we found very few articles, add general finance news as fallback
            seen_urls = {a.get("url") for a in articles if a.get("url")}
            if len(articles) < 3:
                print(
                    f"Only found {len(articles)} articles for {commodity}, "
                    "adding general finance news as fallback"
                )
                for item in data.get("feed", []):
                    if len(articles) >= limit:
                        break
