...
def test_forecast_success(client: TestClient, auth_headers, small_historical_df):
    ...
    with patch(
        "app.data.fetchers.yahoo_fetcher.yf.download", return_value=small_historical_df
    ):
        response = client.post(
            "/api/forecast",
            json={
...
def test_forecast_compare_success(
    client: TestClient, auth_headers, small_historical_df
):
